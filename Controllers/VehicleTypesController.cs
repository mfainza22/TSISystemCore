...
namespace TSISystemCore.Controllers
{
    ...
    public class VehicleTypesController : ControllerBase
    {
        ...
        public IActionResult Put([FromBody] VehicleType model)
        {
            try
            {
                if (!ModelState.IsValid) return InvalidModelStateResult();
                if (!validateEntity(model)) return InvalidModelStateResult();
                if (repository.Get().Count(a => a.VehicleTypeId.Equals(model.VehicleTypeId)) == 0) return NotFound(Constants.ErrorMessages.NotFoundEntity);
        ...
        public IActionResult ValidateCode([FromBody] VehicleType model)
        {
        ...
        [ProducesResponseType(typeof(string), StatusCodes.Status202Accepted)]
        [ProducesResponseType(typeof(StatusCodes), StatusCodes.Status422UnprocessableEntity)]
        public IActionResult ValidateName([FromBody] VehicleType model)
        {
            if (model == null) return NotFound();
            if (General.IsDevelopment) logger.LogDebug(ModelState.ToJson());
            var result = repository.ValidateName(model);
            if (result) return Accepted(true);
            else return UnprocessableEntity(Constants.ErrorMessages.EntityExists("Description"));
        }

        private bool validateEntity(VehicleType model)
        {
            var validCode = repository.ValidateCode(model);
            if (!validCode) ModelState.AddModelError(nameof(VehicleType.VehicleTypeCode), Constants.ErrorMessages.EntityExists("Code"));
            var validName = repository.ValidateName(model);
            if (!validName) ModelState.AddModelError(nameof(VehicleType.VehicleTypeDesc), Constants.ErrorMessages.EntityExists("Description"));
