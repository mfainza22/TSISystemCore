...
namespace TSISystemCore.Controllers
{
    ...
    [ApiController]
    public class BeginningInvAdjsController : ControllerBase
    {
        private readonly IBeginningInvAdjRepository repository;
        private readonly ILogger<BeginningInvAdjsController> logger;
        public BeginningInvAdjsController(ILogger<BeginningInvAdjsController> logger, IBeginningInvAdjRepository repository)
        {
        ...
        public IActionResult Get([FromQuery] int year, int month)
        {
            try
            {
                var model = repository.GetViewsByMonth(year, month);
                return Ok(model);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public IActionResult Post([FromBody] BeginningInvAdj model)
        {
            try
            {
        ...
        [Route("[action]")]
        public IActionResult GetActualWt([FromQuery] int year, [FromQuery] int month, [FromQuery] long categoryId)
        {
            return Ok(repository.GetActualWt(year, month, categoryId));
        }
