...
namespace TSISystemCore.Controllers
{
    ...
    public class SuppliersController : ControllerBase
    {
        ...
        public SuppliersController(ILogger<SuppliersController> logger, ISupplierRepository repository)
        {
        ...
        [HttpGet]
        public IActionResult Get([FromQuery] Supplier parameters = null)
        {
            try
            {
        ...
        public IActionResult Put([FromBody] Supplier model)
        {
            try
            {
                if (!ModelState.IsValid) return InvalidModelStateResult();
                if (!validateEntity(model)) return InvalidModelStateResult();
                if (repository.Get().Count(a => a.SupplierId.Equals(model.SupplierId)) == 0) return NotFound(Constants.ErrorMessages.NotFoundEntity);
        ...
        public IActionResult ValidateCode([FromBody] Supplier model)
        {
        ...
        [ProducesResponseType(typeof(string), StatusCodes.Status202Accepted)]
        [ProducesResponseType(typeof(StatusCodes), StatusCodes.Status422UnprocessableEntity)]
        public IActionResult ValidateName([FromBody] Supplier model)
        {
            if (model == null) return NotFound();
            if (General.IsDevelopment) logger.LogDebug(ModelState.ToJson());
        ...
        private bool validateEntity(Supplier model)
        {
            var validCode = repository.ValidateCode(model);
            if (!validCode) ModelState.AddModelError(nameof(Supplier.SupplierCode), Constants.ErrorMessages.EntityExists("Code"));
            var validName = repository.ValidateName(model);
            if (!validName) ModelState.AddModelError(nameof(Supplier.SupplierName), Constants.ErrorMessages.EntityExists("Name"));
